...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((sum, symb) => sum + symb.charCodeAt(), 0);
}
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((long, word) => {
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].toLowerCase() === str[i]) {
      updatedStr = `${updatedStr}${str[i].toUpperCase()}`;
    } else {
      updatedStr = `${updatedStr}${str[i].toLowerCase()}`;
    }
  }
  return updatedStr;
}
